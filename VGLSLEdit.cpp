...
CGLSLHighlighter::CGLSLHighlighter( QTextDocument *parent ) : QSyntaxHighlighter( parent )
{
	...
	typePatterns << "\\bbool\\b" << "\\bint\\b" << "\\buint\\b"
		<< "\\bfloat\\b" << "\\bdouble\\b"
	...
	*/
	...
	// Then we create a format that we will apply to Qt class names. The class
	// names will be rendered with a dark magenta color and a bold style. We
	// specify a string pattern that is actually a regular expression capturing
	// all Qt class names. Then we assign the regular expression and the specified
	// format to a HighlightingRule object and append the object to our list of rules.

	// We also define highlighting rules for quotations and functions using the
	// same approach: The patterns have the form of regular expressions and are stored
	// in HighlightingRule objects with the associated format.
